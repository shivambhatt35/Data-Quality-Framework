...
# This Section initializes all required Python packages and sets up the basic environment for the DQ Framework.
...
# This cell sets up global variables and configuration settings that will be used throughout the framework. Using a class to manage global state provides better organization than individual global variables.
...
# This sub-section sets up logging configuration for the framework. Proper logging is crucial
# for monitoring and debugging the DQ framework execution.
...
# #### Spark Configuration
# This sub-section configures Spark settings optimized for Microsoft Fabric environment. These settings improve performance and reliability of the DQ framework.
...
def configure_spark():
    """
    Configures Spark session with optimized settings for Microsoft Fabric.
    Includes settings for partitioning, adaptive query execution, and Delta Lake.
    """
    # Partition handling settings
    ...
    # Delta Lake and Parquet optimization
    ...
    logging.info("Spark configuration completed")


# #### Framework Configuration Loading
# This sub-section handles loading and validating the framework configuration from the config files stored in the Lakehouse.
...
def merge_result_tables(
    ...
    join_conditions = [(col(f"target.{key}") == col(f"source.{key}")) | (col(f"target.{key}").isNull() & col(f"source.{key}").isNull()) for key in keys]
...
def extract_keys_values(df):
    ...
    for i in range(4):
        ...
        df = df.withColumn(value_col, expr(f"IF(size(keys_array) > {i}, get_json_object(lower(source_table_attributes), CONCAT('$.', lower(keys_array[{i}]))) , NULL)"))
...
# ## Core Data Operations Module
# This section contains the core data operations used throughout the DQ Framework.These functions handle basic data manipulations, joining operations, and data validation.

# #### Basic DataFrame Operations
# This subsection contains fundamental DataFrame manipulation functions.
...
def union_df(df1, df2):
    """
    Perform a union operation on two DataFrames, combining their rows while matching columns by name.
    ...
    Returns:
        DataFrame: The resulting DataFrame after the union operation
    """
...
def join_dataframes(df1, df2, how='inner', join_columns=None):
    """
    Join two DataFrames on specified or common columns with a specified join type.
    
    Args:
        df1 (DataFrame): The first DataFrame
        df2 (DataFrame): The second DataFrame
        how (str): The type of join to perform (default is 'inner')
        join_columns (str): Comma-separated list of columns to join on (optional)

    Returns:
        DataFrame: The resulting DataFrame after performing the join operation
    """
    try:
        if join_columns:
        ...
        agg_df_aliased = df2.select(
            *[col(c).alias(c + "_alias") if c.lower() in common_columns else col(c) 
...
# This subsection contains functions for data validation and field checking.
...
def is_simple_field(field):
    """
    Check if the field is a simple column name without any expressions or operators.
    
    Args:
        field (str): Field name to check

    Returns:
        bool: True if field is a simple column name, False if it contains operators
    """
    operators = set(' -+*/()')
...
# ##### Validation Of EmiratesID
# This function is used to validate Emirates IDs in a PySpark DataFrame. It performs various checks on the Emirates ID, including length validation, year extraction, and checksum verification using the Luhn algorithm. If a Date of Birth (DOB) column is provided, it also compares the year extracted from the Emirates ID to the year in the DOB column. The function adds status and violation details to the DataFrame based on these validations.
...
# #### Data Type Handling
# This subsection handles data type conversions and casting operations.
...
# This subsection handles table reading and persistence operations.

# In[25]:


def read_and_persist_table(database: str, table_name: str, load_date: int, year_id: int,sub_source:str=None) -> Optional[DataFrame]:
    """
...
# ## Rule Processing Modules
# This section contains functions for loading, configuring, and processing rules

# #### Rule Loading and Configuration
# This subsection handles loading and configuration of rules from the Lakehouse.
...
# This subsection handles the preparation and processing of individual rules.
...
def prepare_rules_for_table(rules: DataFrame, rule_parameters: DataFrame, table_name: str) -> List[PreparedRule]:
    ...
    table_rules = rules.join(
        ...
        *[rules[c] for c in rules.columns if c not in ["system", "sub_system", "domain"]],  # Select all columns from rules except system, sub_system, and domain
        ...
        *[rule_parameters[c] for c in rule_parameters.columns if c not in rules.columns and c not in ["system", "sub_system", "domain"]]
    ...
    for rule_row in table_rules.collect():
        ...
        if rule_row['is_template'] == 1:
            ...
            for column, critical_attribute, model_integrity in paired_columns:
                column = column.strip()
                json_config = generate_template_config(json_string, rule_row, column, critical_attribute, model_integrity)
                ...
                prepared_rule = PreparedRule(
                    ...
                    threshold_string=rule_row['thresholdvalue'] if rule_row['thresholdvalue'] not in (None, "null") else '{}',
                    columns=column,
...
# This subsection contains functions for executing rules and handling their results.
            ...
            if 'conditions' in calc:
                for condition in calc['conditions']:
                    if 'logical_operator' in condition:
                        ...
                        operation = condition['operation']
                        value = replace_parameters_with_values(condition['param'], parameters) if 'param' in condition else condition['value']
                        if is_simple_field(field):
    ...
    for ref_condition in rule_config['reference_conditions']:
        ...
        try:
            if rule_config.get('sub_category') == 'invalid reference values':
                ref_df = (spark.sql(f"SELECT {', '.join(ref_columns)} FROM {ref_db}.{ref_table} WHERE ds_table_name = '{rule_config['table']}' AND ds_column_name = '{rule_config['columns'][0]}'")).toDF(*ref_columns_aliased)
            else:
                ref_df = (spark.sql(f"SELECT {', '.join(ref_columns)} FROM {ref_db}.{ref_table}")).toDF(*ref_columns_aliased)
        except Exception as e:
            logging.error(f"Reference Table {ref_db}.{ref_table} does not exist: {e}, Failed to execute rule_id: {rule_id}")
            return None
        ...
        df_no_match = applicable_data.join(ref_df, on=join_conditions, how='left_anti')\
            ...
            .withColumn("New_Violation_Details", lit(f"Consistency check failed for columns: {', '.join(rule_config.get('columns', df.columns))}"))
...
def process_rules_for_table(prepared_rules: List[PreparedRule], original_df: DataFrame) -> Tuple[DataFrame, Dict[str, List[str]]]:
    """
...
def process_rules_by_table(rules: DataFrame, rule_parameters: DataFrame, load_date: int, year_id: int, load_only_anomalies_in_detailed_logs: str, lakehouse_layer: str, sub_source: str) -> Dict[str, Dict[str, List[str]]]:
    """
...
# #### Rule Result Summary Generation
# This subsection handles formatting and processing of rule execution results
...
# #### Detailed Data Quality (DQ) Logs Generation
...
# ## Main Function Of DQ Engine Execution
# This Section, defines the main function. This function executes the DQ engine process for data quality (DQ) or business rule checks. It coordinates the loading of configurations, preparation and processing of rules, and saving of results, ensuring a comprehensive data quality and rule validation process.

# In[70]:


def main(rule_type, rules_to_be_executed, systems_to_be_executed, sub_systems_to_be_executed, domains_to_be_executed, tables_to_be_executed, tables_to_be_excluded, load_only_anomalies_in_detailed_logs, number_of_output_files, load_date, year_id, lakehouse_layer,sub_source,run_date,Pipeline_Name):
    """
    Main function to execute the rule engine process for data quality or business rule checks.
    
    Args:
        rule_type (str): The type of rules to be executed DQ or Business.
        rules_to_be_executed (str): Comma-separated list of specific rules to be executed, or 'all'.
        systems_to_be_executed (str): Comma-separated list of systems to be executed, or 'all'.
        sub_systems_to_be_executed (str): Comma-separated list of sub-systems to be executed, or 'all'.
        domains_to_be_executed (str): Comma-separated list of domains to be executed, or 'all'.
        tables_to_be_executed (str): Comma-separated list of tables to be executed, or 'all'.
        tables_to_be_excluded (str): Comma-separated list of tables to be excluded.
        load_only_anomalies_in_detailed_logs (str): Flag to determine if only anomalies should be loaded in detailed logs.
        number_of_output_files (int): The number of output files for partitioning the anomalies.
        load_date (str): The load date for data filtering.
    ...
    try: 
        setup_logging()
        logging.info(f"Started DQ Engine to Check {rule_type}.....")
        logging.info(f"Initializing the DQ Framework")
        log_execution(
            target_table="job_execution_logs",
            notebook_name="DQ_Framework",
            pipeline_name=Pipeline_Name,
            layer=lakehouse_layer,
            system=systems_to_be_executed,
            sub_system=sub_systems_to_be_executed,
            status="Running",
            details="Execution started."
        )
        load_framework_config()
        refresh_config_on_lake()
        configure_spark()
        
        rules, rule_threshold, rule_parameters = load_rule_configurations(rule_type, rules_to_be_executed, systems_to_be_executed, sub_systems_to_be_executed, domains_to_be_executed, tables_to_be_executed, lakehouse_layer, tables_to_be_excluded)
        rules = prepare_rules(rules, rule_threshold)

        if rules.count() == 0:
            logging.info("No Config found for specified rule, system, sub_system, domain. Kindly check the config in Lakehouse")
            return
...
# ## Job Parameters
# This section defines all the parameters required to run the DQ Engine on General Education Data

# In[71]:


rule_type = "Data Quality"
rules_to_be_executed ='all'
systems_to_be_executed = 'GE'
sub_systems_to_be_executed = 'MOE-PUBLIC'
domains_to_be_executed = 'all'
tables_to_be_executed = 'all'
tables_to_be_excluded = ''
load_only_anomalies_in_detailed_logs = 'true'
load_date=20241225
year_id=0
number_of_output_files = 5
sub_source='MOE-PUBLIC'
lakehouse_layer='bronze'
run_date=None
Pipeline_Name=None


# ## Start DQ Engine Execution
# This Section is the starting point of DQ Execution.

# In[72]:


main(rule_type, rules_to_be_executed, systems_to_be_executed, sub_systems_to_be_executed, domains_to_be_executed, tables_to_be_executed, tables_to_be_excluded, load_only_anomalies_in_detailed_logs, number_of_output_files, load_date, year_id, lakehouse_layer, sub_source, run_date, Pipeline_Name)
